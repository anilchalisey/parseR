#' Pipeline for edgeR analysis
#'
#' Function to perform entire edgeR analysis using edgeR-QLF.  It begins with
#' a SummarizedExperiment object and ends with list of differential genes and
...
#'
#' @return
#' Results of edgeR differential analysis.
#'
#' @import edgeR
...
edger_pipe <- function(se = NULL,
  ...
  ############
  edger_results <- run_edger(dge = dge, design = design.matrix,
  ...
  for (i in 1:length(out)) {
    write.table(out[[i]], row.names = F, col.names = TRUE,
                quote = F, sep = "\t",
                file = paste0("DEgenes_edger_", names(out)[i], ".txt"))
  }
